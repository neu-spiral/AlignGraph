...
def between_cluster_edges(G, cluster_ind):
    ...
    n_edges = []
    edge_orig = G.edges()
    # print("****testing next step****")
    print("# of edges", len(edge_orig))
    # print("cluster_ind[0], cluster_ind[1]", cluster_ind[0], cluster_ind[1])
...
def binarized_P_adj(A_cluster, P_opt, i):
    ...
    adj_x = np.matmul(P_binary.T, A_cluster)
    adj = np.matmul(adj_x, P_binary)
...
class AP_cent(object):
    ...
    def __init__(self, num_clusters, num_tr, A_thr, kmeans):
    ...
    def final_graphs(self, A, graph_train):
        ...
        print("unique elements:", np.unique(A_cent_sprs))
        print("A_cent_sprs", A_cent_sprs)
        # print("# of non-zero elements of sparse matrix", np.count_nonzero(A_cent_sprs))
        A_cent_rec = np.zeros((len(A_cent_sprs), len(A_cent_sprs)))
        A_cent = center_cls_ave(A_total)
        ...
        for t in thr:
            ...
            tmp_vecs = pool.starmap(
                binarized_vec,
                [(A_cent[i], self.A_thr) for i in range(len(A_cent))],
            )
            pool.close()
            pool.join()
            A_cent_rec = np.asarray([tmp_vecs[k] for k in range(len(A_cent))])
            print(
                "# of non-zero elements of sparse matrix after binarizing",
                np.count_nonzero(A_cent_sprs),
            )
...
class AP_align(object):
    ...
    def __init__(self, A0, num_clusters, num_tr, A_thr):
...
if __name__ == "__main__":
    ...
    if args.center:
        ...
        obj = AP_cent(num_clusters, num_tr, 0.43, kmeans)
        pool_size = mp.cpu_count()
        ...
        np.save("A_cent_parall", A_cent_rec_cls)
        np.save("A_total_parall", A_total_cls)

        # num_tr= num_tr+1
        while len(A_cent_rec_cls) > num_tr + 1:
            print("*" * 50)
        ...
        if len(A_total_cls) > 1:
            print("len(A_total_cls)", len(A_total_cls))
            num_tr = len(A_total_cls)
            obj = AP_cent(num_clusters, num_tr, 0.43, kmeans)
        ...
        # kmeans = KMeans(n_clusters=num_clusters, embedding_method=GSVD(3))
        A_cent_rec_cls = np.load("A_cent_parall.npy")
        print(
            "len(A_cent_rec_cls)",
            len(A_cent_rec_cls),
