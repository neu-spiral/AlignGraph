...
class GRANData(object):
    ...
    def _get_graph_data(self, G):
        ...
        if self.num_canonical_order == 5:
            ...
            if self.node_order == "degree_decent":
                ...
                print("adj_4", adj_4, len(adj_4))
                adj_list = [adj_4]
            elif self.node_order == "k_core":
                adj_list = [adj_5]
            elif self.node_order == "Alignment":
                print("using alignment")
                adj_list = [adj_0]
                print("adj_0", adj_0, len(adj_0))
    ...
    def __getitem__(self, index):
        ...
        # print("len(self.file_names)", len(self.file_names))
        # print("index", index)
        # load graph
        ...
        for ff in range(self.num_fwd_pass):
            ...
            for ii in range(len(adj_list)):
                ...
                for jj in range(0, num_nodes, S):
                    ...
                    adj_block1 = adj_block
                    adj_block = np.tril(adj_block, k=-1)
                    adj_block = adj_block + adj_block.transpose()
                    # print("adj_block1-adj_block", np.amax(adj_block1-adj_block), adj_block1-adj_block)
