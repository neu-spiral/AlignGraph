...
def clustering_stats(graph_ref_list, graph_pred_list, bins=100, is_parallel=True):
    ...
    print("len(sample_ref), len(sample_pred)", len(sample_ref), len(sample_pred))
...
def motif_stats(
    ...
    print(
        "len(graph_ref_list), len(graph_pred_list_remove_empty)",
        len(graph_ref_list),
        len(graph_pred_list_remove_empty),
    )
    graph_pred_list_remove_empty = [
        G for G in graph_pred_list if not G.number_of_nodes() == 0
    ]
...
def orbit_stats_all(graph_ref_list, graph_pred_list):
    ...
    # print("len(graph_ref_list), len(graph_pred_list_remove_empty)", len(graph_ref_list), len(graph_pred_list_remove_empty))
    ...
    print(
        "len(total_counts_ref), len(total_counts_pred)",
        len(total_counts_ref),
        len(total_counts_pred),
    )
    mmd_dist = compute_mmd(
        total_counts_ref,
        total_counts_pred,
