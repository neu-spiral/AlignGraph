...
def snapshot(model, optimizer, config, step, gpus=[0], tag=None, scheduler=None):
    ...
    torch.save(
        ...
        os.path.join(
            ...
            if tag is not None
            else "model_snapshot_{:07d}.pth".format(step),
        ),
    )
    ...
    config_save = edict(yaml.full_load(open(save_name, "r")))
    print("config_save", config_save)
    config_save.test.test_model_dir = config.save_dir
    print("config_save.test.test_model_dir", config_save.test.test_model_dir)
    config_save.test.test_model_name = (
        "model_snapshot_{}.pth".format(tag)
        if tag is not None
        else "model_snapshot_{:07d}.pth".format(step)
    )
    yaml.dump(edict2dict(config_save), open(save_name, "w"), default_flow_style=False)
    print("config_save.test.test_model_name", config_save.test.test_model_name)
    return config_save
...
def load_model(model, file_name, device, optimizer=None, scheduler=None):
    ...
    # print("*******model_snapshot******", model_snapshot["model"])
    model.load_state_dict(model_snapshot["model"], strict=False)
    # model.load_state_dict(model_snapshot["model0"])
