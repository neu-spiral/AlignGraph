...
class validation_score:
    ...
    def ref_graph(self):
        ...
        print("len(self.graph_pred_list)", len(self.graph_pred_list))
        for i in range(len(self.graph_pred_list)):
            print(
                "i, self.graph_pred_list[i].number_of_nodes(), self.graph_pred_list[i].number_of_edges()",
                i,
                self.graph_pred_list[i].number_of_nodes(),
                self.graph_pred_list[i].number_of_edges(),
            )
            """
            adj = np.zeros(
                (
    ...
    def score(self):
        ...
        print(
            "np.mean(self.assort_ref), np.mean(self.assort_pred), np.mean(self.claw_ref), np.mean(self.claw_pred), np.mean(self.wedge_ref), np.mean(self.wedge_pred)",
            np.mean(self.assort_ref),
            np.mean(self.assort_pred),
            np.mean(self.claw_ref),
            np.mean(self.claw_pred),
            np.mean(self.wedge_ref),
            np.mean(self.wedge_pred),
        )
