...
def train_rnn_epoch(
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        # print("output.hidden", len(output.hidden), (output.hidden[0])) #edge output hidden
        y_pred = output(output_x, pack=True, input_len=output_y_len)
        # print("edge level representation y_pred", len(edge_hidden)) #edge output hidden
        # print("y_pred before sigmoid", y_pred, len(y_pred), len(y_pred[0]))
        y_pred = F.sigmoid(y_pred)
        # print("y_pred after sigmoid", len(y_pred), len(y_pred[0]))
        # clean
        y_pred = pack_padded_sequence(y_pred, output_y_len, batch_first=True)
        y_pred = pad_packed_sequence(y_pred, batch_first=True)[0]
        output_y = pack_padded_sequence(output_y, output_y_len, batch_first=True)
        output_y = pad_packed_sequence(output_y, batch_first=True)[0]
        # print("y_pred final", len(output_y), len(output_y[0]), len(output_y[0][0]))
        # use cross entropy loss
...
def test_rnn_epoch(epoch, args, rnn, output, test_batch_size=16):
    ...
    for i in range(test_batch_size):
        adj_pred = decode_adj(y_pred_long_data[i].cpu().numpy())
        # print("test y_pred_long_data", len(y_pred_long_data), len(y_pred_long_data[0]), y_pred_long_data)
        G_pred = get_graph(adj_pred)  # get a graph from zero-padded adj
        G_pred_list.append(G_pred)
...
def train(args, dataset_train, rnn, output):
    ...
    if args.load:
        fname = (
            args.model_save_path + args.fname + "lstm_" + str(args.load_epoch) + ".dat"
        )
    ...
    save_graph_list(G_test, "graph_test")
    print("len(G_test)", len(G_test))
    return G_test, G_pred
